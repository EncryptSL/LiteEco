...
class AdminEconomyMoneyWithdrawListener(private val liteEco: LiteEco) : Listener {
    ...
    fun onAdminEconomyMoneyWithdraw(event: AdminEconomyMoneyWithdrawEvent) {
        ...
        if (!liteEco.api.hasAccount(target)) {
            sender.sendMessage(
                ModernText.miniModernText(liteEco.locale.getMessage("messages.account_not_exist"),
                TagResolver.resolver(Placeholder.parsed("account", target.name.toString()))))
            return
        }

        if (!liteEco.api.has(target, money)) {
            sender.sendMessage(ModernText.miniModernText(liteEco.locale.getMessage("messages.sender_error_enough_pay")))
            return
        }
        ...
        if (sender.name == target.name) {
            sender.sendMessage(
                ModernText.miniModernText(
                    liteEco.locale.getMessage("messages.self_withdraw_money"),
        ...
        sender.sendMessage(
            ModernText.miniModernText(
                liteEco.locale.getMessage("messages.sender_success_withdraw"),
        ...
        if (target.isOnline) {
            ...
            target.player?.sendMessage(
                ModernText.miniModernText(
                    liteEco.locale.getMessage("messages.target_success_withdraw"),
